module.exports = {
  '100': {
    Name: 'Continue(継続)',
    Desc: 'クライアントはリクエストを継続できます。サーバーがリクエストの最初の部分を受け取り、まだ拒否していないことを示します。 例として、クライアントがExpect:100-continueヘッダをつけたリクエストを行い、それをサーバーが受理した場合に返されます。',
  },
  '101': {
  ...
  '500': {
    Name: 'Internal Server Error(サーバ内部エラー)',
    Desc: 'サーバー内部にエラーが発生した場合に返されます。例えば、CGIとして動作させているプログラムに文法エラーがあったり、設定に誤りがあった場合などです。',
  },
  '501': {
    Name: 'Not Implemented(実装されていない)',
    Desc: '実装されていないメソッドを使用した場合に返されます。例えば、ウェブダブが実装されていないサーバーに対してウェブダブで使用するメソッド（MOVEやCOPY）を使用した場合などです。。',
  },
  '502': {
